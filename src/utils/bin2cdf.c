                       ...
                       const Coord_array *array)
{
  ...
  error(rc);
  snprintf(s,STRING_LEN,"bin2cdf %s",bin_filename);
  ...
  if(ispft)
  {
    rc=nc_def_dim(cdf->ncid,name,header.nbands,&pft_dim_id);
    error(rc);
    dim[0]=time_dim_id;
    dim[1]=pft_dim_id;
    dim[2]=lat_dim_id;
...
int main(int argc,char **argv)
{
  ...
  compress=0;
  swap=FALSE;
  res.lon=res.lat=header.cellsize=0.5;
  ...
  for(iarg=1;iarg<argc;iarg++)
    if(argv[iarg][0]=='-')
    {
      ...
      else if(!strcmp(argv[iarg],"-cellsize"))
      {
        ...
        res.lon=res.lat=header.cellsize=cellsize;
      }
  ...
  if(index==NULL)
    return EXIT_FAILURE;
  free(grid);
  cdf=create_cdf(argv[iarg+3],argv[iarg+2],argv[iarg],units,descr,(isshort) ? LPJ_SHORT : LPJ_FLOAT,header,ispft,compress,index);
  if(cdf==NULL)
    return EXIT_FAILURE;
  if(isshort)
  {
