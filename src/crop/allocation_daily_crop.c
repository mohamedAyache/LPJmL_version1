...
/**     Last change: 24.02.2009  by S.Schaphoff                           **/
/**     Last change: 15.06.2009  by S.Schaphoff                           **/
/**     Last change: $Date::                                            $ **/
/**     By         : $Author::                                 $          **/
/**                                                                       **/
/***************************************************************************/
        ...
        )
{
    ...
    //unlike grass.ind and tree.ind crop.ind unit is gC/m2
    //we take all of crop->ind.root because it is equal to the daily increment of crop->ind.root
    if(crop->ind.root>0){crop->uptake.root += crop->ind.root;}else {crop->residue.root += -crop->ind.root;}
    if(crop->ind.so>0){crop->uptake.so += crop->ind.so;}else {crop->residue.so += -crop->ind.so;}
    if(crop->ind.leaf>0){crop->uptake.leaf += crop->ind.leaf;}else {crop->residue.leaf += -crop->ind.leaf;}
