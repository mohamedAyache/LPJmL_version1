...

{
    ...
    /*barrier.n: reduction of uptake */
    //grass->uptake.root -= m_kill.root; printf("killroot\n");//negative uptake is taken care of by mineralisation (residue, seen below)

    litter->ag[pft->litter].trait.leaf+=m_kill.leaf;
    // barrier.n
    grass->residue.leaf += m_kill.leaf;
    update_fbd_grass(litter,pft->par->fuelbulkdensity,m_kill.leaf);
    litter->bg[pft->litter]+=m_kill.root;
    // barrier.n
    grass->residue.root +=m_kill.root;
    fpc_grass(pft);
} /* of 'light_grass' */
