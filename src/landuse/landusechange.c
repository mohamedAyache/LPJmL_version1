        ...
        )
{
    ...
    if(s!=NOT_FOUND)
    {
        // s is the index of the natural stand
        ...
        if(natstand->frac>minnatfrac)
        {
            ...
            // pos is the index of the new stand (first, is natural)
            ...
            // barrier.n: deforest: update soil concentration
            nit_deforest_updatesoil(natstand, cutstand, difffrac);

            if(difffrac+epsilon>=natstand->frac)
            {
                // if the fraction to remove exceeds the fraction of natural stand
                // remove natural stand and move index
                delstand(cell->standlist,s);
                pos=s;
            }
            else
                // if natural stand still removes, update of natural stannd fraction
                natstand->frac-=difffrac;
            if(!timberharvest)
            {
                // note: if a setaside stannd already exists, the current stand is mixed with
                // the existing setaside stand, and the function returns True. Then, we remove the stand.
