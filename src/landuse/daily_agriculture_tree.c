        ...
        ) /* returns runoff (mm) */
{
    ...
    if(irrig_apply>epsilon)
    {
        ...
        output->cft_irrig_events[agtree(ncft)+stand->pft_id-npft+config->nagtree+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]++;
    }
    ...
    foreachpft(pft,p,&stand->pftlist)
    {
        ...
        if(stand->frac>0.0 && gp_pft[getpftpar(pft,id)]>0.0)
        {
            ...
            output->pft_gcgp[(npft-config->nbiomass-config->nagtree)+agtree(ncft)+stand->pft_id-npft+config->nagtree+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=gc_pft/gp_pft[getpftpar(pft,id)];
        }

        npp=npp(pft,gtemp_air,gtemp_soil,gpp-rd);

        /* barrier.n: copy of NELA's code concerning harvest of
         * fruits */
        if(istree(pft) && npp>0)
        {
            tree=pft->data;
