                        ...
                        )            /* returns runoff (mm) */
{
  ...
  if(data->irrigation && data->irrig_amount>epsilon)
  {
    ...
    else
    {
      ...
      if(config->pft_output_scaled)
        stand->cell->output.cft_airrig[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=irrig_apply*stand->cell->ml.landfrac[1].biomass_grass;
      else
        stand->cell->output.cft_airrig[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=irrig_apply;
    }
  }
  ...
  if(irrig_apply>epsilon)
  {
    ...
    output->cft_irrig_events[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]++; /* id is consecutively counted over natural pfts, biomass, and the cfts; ids for cfts are from 12-23, that is why npft (=12) is distracted from id */
  }
 ...
 */
    ...
    if(gp_pft[getpftpar(pft,id)]>0.0)
    {
      ...
      {
        output->gcgp_count[(npft-config->nbiomass)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]++;
        output->pft_gcgp[(npft-config->nbiomass)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=gcgp;
      }
    }
    ...
    if(config->pft_output_scaled)
      output->pft_npp[(npft-config->nbiomass)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=npp*stand->cell->ml.landfrac[data->irrigation].biomass_grass;
    else
      output->pft_npp[(npft-config->nbiomass)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=npp;
  } /* of foreachpft */
  ...
  if(isphen)
  {
    ...
    if(data->irrigation)
    {
      ...
      if(config->pft_output_scaled)
      {
        stand->cell->output.cft_conv_loss_evap[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]-=(data->irrig_stor+data->irrig_amount)*(1/data->ec-1)*data->conv_evap*stand->cell->ml.landfrac[1].biomass_grass;
        stand->cell->output.cft_conv_loss_drain[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]-=(data->irrig_stor+data->irrig_amount)*(1/data->ec-1)*(1-data->conv_evap)*stand->cell->ml.landfrac[1].biomass_grass;
      }
      else
      {
        stand->cell->output.cft_conv_loss_evap[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]-=(data->irrig_stor+data->irrig_amount)*(1/data->ec-1)*data->conv_evap;
        stand->cell->output.cft_conv_loss_drain[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]-=(data->irrig_stor+data->irrig_amount)*(1/data->ec-1)*(1-data->conv_evap);
      }
 ...
 */
    ...
    if(gp_pft[getpftpar(pft,id)]>0.0)
    {
      ...
      if(stand->cell->ml.landfrac[data->irrigation].biomass_grass>0.0)
      {
        output->gcgp_count[(npft-config->nbiomass-config->nagtree)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]++;
        output->pft_gcgp[(npft-config->nbiomass-config->nagtree)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=gcgp;
      }
    }
    npp=npp_grass(pft,gtemp_air,gtemp_soil,gpp-rd);
    output->mnpp+=npp*stand->frac;
    output->mgpp+=gpp*stand->frac;
    output->mfpar+=(fpar(pft)*stand->frac*(1.0/(1-stand->cell->lakefrac)))*(1-beta);
    output->cft_fpar[(npft-config->nbiomass-config->nagtree)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=(fpar(pft)*stand->cell->ml.landfrac[data->irrigation].biomass_grass*(1.0/(1-stand->cell->lakefrac)))*(1-beta);

    if(config->pft_output_scaled)
      output->pft_npp[(npft-config->nbiomass-config->nagtree)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=npp*stand->cell->ml.landfrac[data->irrigation].biomass_grass;
    else
      output->pft_npp[(npft-config->nbiomass-config->nagtree)+rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]+=npp;
  } /* of foreachpft */
  ...
  if(config->pft_output_scaled)
    output->pft_harvest[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)].harvest+=harvest.harvest*stand->cell->ml.landfrac[data->irrigation].biomass_grass;
  else
    output->pft_harvest[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)].harvest+=harvest.harvest;

    /* harvested area */
  if(isphen)
    output->cftfrac[rbgrass(ncft)+data->irrigation*(ncft+NGRASS+NBIOMASSTYPE+config->nagtree)]=stand->cell->ml.landfrac[data->irrigation].biomass_grass;
