                             ...
                             )
{
  ...
  {
    if(isroot(*config))
      fprintf(stderr,"ERROR412: Incompatible resolution %g for longitude in '%s', not %g.\n",dim[1]-dim[0],filename,config->resolution.lon);
    free(dim);
    nc_close(input->ncid);
    free(input);
    return NULL;
  ...
  if(rc)
  {
    free(dim);
    if(isroot(*config))
      fprintf(stderr,"ERROR403: Cannot read %s in '%s': %s.\n",
              name,filename,nc_strerror(rc));
    nc_close(input->ncid);
    free(input);
    return NULL;
...
size_t getindexinput_netcdf(const Input_netcdf input,
                            const Coord *coord,const Coord *resolution)
{
  size_t offsets[2];
  if(input->offset)
    offsets[0]=input->offset-(int)((coord->lat-input->lat_min)/resolution->lat+0.5);
  else
    offsets[0]=(int)((coord->lat-input->lat_min)/resolution->lat+0.5);
  offsets[1]=(int)((coord->lon-input->lon_min)/resolution->lon+0.5);
                      ...
                      const Coord *coord,const Coord *resolution)
{
  ...
  switch(input->type)
  {
    case LPJ_FLOAT:
      ...
      {
        fputs("ERROR415: Cannot read float data for cell (",stderr);
        fprintcoord(stderr,*coord);
        fprintf(stderr,"): %s.\n",nc_strerror(rc));
        return TRUE;
      }
      if(f==input->missing_value.f)
      {
        fputs("ERROR423: Missing value for cell (",stderr);
        fprintcoord(stderr,*coord);
        fputs(").\n",stderr);
        return TRUE;
      }
      ...
      {
        fputs("ERROR423: Invalid value for cell (",stderr);
        fprintcoord(stderr,*coord);
        fputs(").\n",stderr);
        return TRUE;
      }
    ...
    case LPJ_DOUBLE:
      ...
      {
        fputs("ERROR415: Cannot read double data for cell (",stderr);
        fprintcoord(stderr,*coord);
        fprintf(stderr,"): %s.\n",nc_strerror(rc));
        return TRUE;
      }
    ...
    case LPJ_INT:
      ...
      {
        fputs("ERROR415: Cannot read int data for cell (",stderr);
        fprintcoord(stderr,*coord);
        fprintf(stderr,"): %s.\n",nc_strerror(rc));
        return TRUE;
      }
    ...
    case LPJ_SHORT:
      ...
      {
        fputs("ERROR415: Cannot read short data for cell (",stderr);
        fprintcoord(stderr,*coord);
        fprintf(stderr,"): %s.\n",nc_strerror(rc));
        return TRUE;
      }
                         ...
                         const Coord *coord,const Coord *resolution)
{
  ...
  if(input->offset)
    offsets[0]=input->offset-(int)((coord->lat-input->lat_min)/resolution->lat);
  else
    offsets[0]=(int)((coord->lat-input->lat_min)/resolution->lat);
  offsets[1]=(int)((coord->lon-input->lon_min)/resolution->lon);
  switch(input->type)
  {
    case LPJ_INT:
                           ...
                           const Coord *coord,const Coord *resolution)
{
  ...
  {
    fprintf(stderr,"ERROR423: Missing value for cell (");
    fprintcoord(stderr,*coord);
    fputs(").\n",stderr);
    return TRUE;
  }
