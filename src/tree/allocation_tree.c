        ...
        )                /* returns TRUE on death */
{
    ...
    /* barrier.n: updates of uptake for leaf, root, sapwood (gN/m2)*/
    if (tinc_ind.leaf>0) {tree->uptake.leaf += tinc_ind.leaf*pft->nind;} else /*death of leaves*/ {tree->residue.leaf += -tinc_ind.leaf*pft->nind;}//gC/plant * nbofplants/m2
    if (tinc_ind.root>0) {tree->uptake.root += tinc_ind.root*pft->nind;} else /*death of roots*/ {tree->residue.root += -tinc_ind.root*pft->nind;}//gC/plant * nbofplants/m2
    if (tinc_ind.sapwood>0) {tree->uptake.sapwood += tinc_ind.sapwood*pft->nind;} else {tree->residue.sapwood += -tinc_ind.sapwood*pft->nind;}//gC/plant * nbofplants/m2
