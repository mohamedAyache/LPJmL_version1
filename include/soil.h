...
extern Real soildepth[NSOILLAYER]; //this seems to be not the depth but rather the width of layer l
...
typedef struct
{
  ...
  int count;
  Real YEDOMA;          /* g/m2 */
  Litter litter;      /* Carbon Litter pool */
  Real decomC[NSOILLAYER];
  Real rw_buffer;             /* available rain water amount in buffer (mm) */

  Real NO3[LASTLAYER];      /* barrier.n: NO3 (DIN) per soillayer gN/m2*/
  Real NH4[LASTLAYER];      /* barrier.n: NH4 (DIN) per soillayer gN/m2 */
  Real PO4[LASTLAYER];      /* barrier.n: PO4 (DIP) per soillayer gP/m2 */
  Real PO4adsorbed[LASTLAYER]; /*PO4 adsorbed (unavailable for uptake) per soillayer gP/m2*/
  Real DOC[LASTLAYER];      /* barrier.n: DOC per soillayer g/m2       */
  Real DOP[LASTLAYER];      /* barrier.n: DOP per soillayer g/m2       */
  Real DON[LASTLAYER];      /* barrier.n: DON per soillayer g/m2       */
  /* residues for N and P */
  Real RESC[LASTLAYER];    /* gC/m2 */
  Real RESN[LASTLAYER];    /* gN/m2 */
  Real RESP[LASTLAYER];    /* gP/m2 */
  Real DECC[LASTLAYER];    /* g/m2 */
} Soil;
