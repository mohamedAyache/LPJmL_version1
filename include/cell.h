...
struct cell
{
  ...
  /* barrier.n: concentration of nutrients in water */
  Real water_temp ;   /* water temperature (seasonal cycle at this time)*/
  Real preccheck[4] ;  /* check whether it rained during the last three days (prec[3]=1) */
  Real surf_crop;  /* total crop fraction within the current cell */
  Real surf_grass;  /* total grassland fraction in the current cell */
  Real surf_for;  /* total natural (forest) fraction in the current cell */
};
...
extern void update_daily(Cell *,Real,Real,Dailyclimate,int,
                         int,int,int,int, Bool,Bool,const Config *, int);
...
extern void iterateyear_river(Outputfile *,Cell [],Input,
                              Real,int,int,int,const Config *, Bool);
