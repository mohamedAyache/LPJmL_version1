...
typedef struct
{
  ...
  Real (*daily)(Stand *,Real,Dailyclimate,int,Real,
                ...
                Real,Real,int,int,int,Bool,Bool,const Config *, int);
  Bool (*annual)(Stand *,int,int,
                 Real,int,Bool,const Config *);

  void (*uptake) (Stand *, const Config *);
...
struct stand
{
  ...
  int lastdate_fert;          /* barrier.n: date of the last fertilisation on the current stand */
  int lastdate_man;          /* barrier.n: date of the last manure on the current stand */
  Real annualUptakeP[NSOILLAYER];         // add this variable to save the total annual uptake g/m2
  Real annualResP[NSOILLAYER];         // add this variable to save the total annual uptake g/m2
  Real annualUptakeN[NSOILLAYER];         // add this variable to save the total annual uptake g/m2
  Real annualResN[NSOILLAYER];         // add this variable to save the total annual uptake g/m2
};
...
extern void waterbalance(Stand *,Real [BOTTOMLAYER],Real [BOTTOMLAYER],Real *,Real *,Real,Real,
                         Real,Real *,Bool);
extern Real infil_perc_irr(Stand *,Real,Real *,Bool, const Config *,int);
extern Real infil_perc_rain(Stand *,Real,Real *,Bool, const Config *,int);
